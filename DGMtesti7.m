...
pext=7.731; %773.1Pa=kg*m^-1*s^-2=0.01*kg*cm^-1*s^-2  5.58mmHg
%pext=1.021*10^(-3);
%pext=1021;

betaL=304; betaR=304; beta3=304;
AL0=0.106; AR0=0.106; A30=0.106;
%rho=0.001021;
%rho=10.21;
rho=0.001;
%rho=1.035*10^(-3);

c0=sqrt(betaL/(2*rho*AL0))*nthroot(AL0,4)
...
for i=1:nT
    ...
    UR=(-1).^(0:P)*RUU((N-1)*(P+1)+1:N*(P+1));
    upwinded=FastNewtonRhapson([AL UL AR UR],10,AL,UL,AR,UR,AL0,AR0,betaL,betaR,rho);
    uwa=0.5*(upwinded(1)+upwinded(3));
    uwu=0.5*(upwinded(2)+upwinded(4));
    ...
    resultsA(:,i+1)=RA + deltaT*RHS(N,RA,RU,@F1,@S1,qW,qP,LPminus,LPplus,LP,epsilon,bcA(i,deltaT),bcU(i,deltaT),uwa,uwu,betaL,AL0,rho, ...
        currentUpper, nextLower, currentLower, previousUpper,iM,iJ,LPder);
    resultsU(:,i+1)=RU + deltaT*RHS(N,RA,RU,@(a,u) F2(a,u,pext,betaL,AL0,rho),@(a,u) S2(a,u,rho),qW,qP,LPminus,LPplus,LP,epsilon,bcA(i,deltaT),bcU(i,deltaT),uwa,uwu,betaL,AL0,rho, ...
        currentUpper, nextLower, currentLower, previousUpper,iM,iJ,LPder);
    
    
    resultsAA(:,i+1)=RAA + deltaT*RHS(N,RAA,RUU,@F1,@S1,qW,qP,LPminus,LPplus,LP,epsilon,uwa,uwu,UW(1),UW(2),betaR,AR0,rho, ...
        currentUpper, nextLower, currentLower, previousUpper,iM,iJJ,LPder);
    resultsUU(:,i+1)=RUU + deltaT*RHS(N,RAA,RUU,@(a,u) F2(a,u,pext,betaR,AR0,rho),...
        @(a,u) S2(a,u,rho),qW,qP,LPminus,LPplus,LP,epsilon,uwa,uwu,UWA,UWU,betaR,AR0,rho, ...
        currentUpper, nextLower, currentLower, previousUpper,iM,iJJ,LPder);
    ...
    resultsAAA(:,i+1)=RAAA + deltaT*RHS(N,RAAA,RUUU,@F1,@S1,qW,qP,LPminus,LPplus,LP,epsilon,UW(3),UW(4),A30,0,beta3,A30,rho, ...
        currentUpper, nextLower, currentLower, previousUpper,iM,iJJJ,LPder);
    resultsUUU(:,i+1)=RUUU + deltaT*RHS(N,RAAA,RUUU,@(a,u) F2(a,u,pext,beta3,A30,rho),...
        @(a,u) S2(a,u,rho),qW,qP,LPminus,LPplus,LP,epsilon,UWA,UWU,A30,0,beta3,A30,rho, ...
        currentUpper, nextLower, currentLower, previousUpper,iM,iJJJ,LPder);
...
MemoryAA=zeros(N*(P+1),4);
MemoryUU=zeros(N*(P+1),4);
MemoryAAA=zeros(N*(P+1),4);
MemoryUUU=zeros(N*(P+1),4);


fps=24;
frameT=floor(0.05*nT/(fps*(endT-startT)));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%Plotting%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
multiplier=1/1.3332;
...
figure
for i=100:100:1500%floor(nT/10):nT
    tic
    A=reshape(resultsA(:,i),P+1,N);
    U=reshape(resultsU(:,i),P+1,N);
    AA=reshape(resultsAA(:,i),P+1,N);
...
function y=bcA(i,deltaT)
    ...
    %y=10+i*deltaT;
    %y=0;
    y=(0.106/304*(pressure(i,deltaT)-5.8*1.3332368) + 0.106^0.5)^2; %5.58mmHg=773.100Pa
end 
function y=bcU(i,deltaT)
    %y=1+deltaT*i;
    %y=1;
    %y=1.5-0.5*cos(2*deltaT*(i-1));
    %y=1*(1-cos(8*deltaT*(i-1)));
    y=1;
end 

function p=pressure(i,deltaT)
    p=1.33322368*(100-20*cos(16*deltaT*(i)));%80-120 mmHg 1.33322368*
end
